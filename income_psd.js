...
(async () => {
	...
	for (const [idx, data] of master_loop.entries()) {
		...
		// https://ag.ufa6666.com/_Part_Sub/SubAccsWinLose2.aspx?role=ag&userName=ufruu01&from=06/06/2022&to=06/12/2022&catId=1,8,11,12,13,14,15,16,20,21,22,23,25,26,27,29,30,31,32,33&userID=ufruu&checkAll=True
		await page.goto(agtest + `/_Part_Sub/SubAccsWinLose2.aspx?role=ag&userName=` + data.master + `&from=06/06/2022&to=06/12/2022&userID=ufruu&checkAll=True`, {
			waitUntil: 'networkidle2'
		})
		await delay(2000);

		// await page.goto(agtest + `/_Part_Sub/SubAccsWinLose2.aspx?role=ag&userName=` + data.username + `&from=` + start + `&to=` + end + `&userID=ufruu&checkAll=True`, {
		// 	waitUntil: 'networkidle2'
		// })
