...
async function handleGet(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    if (id) {
      const record = await prisma.masterChangePasswordDB.findFirst({
        where: { id: String(id) },
      })
      if (!record) {
    ...
    if (keyword) {
      where.OR = [
        ...
        { masterLogin: { contains: String(keyword), mode: 'insensitive' } },
      ]
    }
    ...
    const [records, total] = await Promise.all([
      prisma.masterChangePasswordDB.findMany({
        where,
        orderBy: { createdAt: 'desc' },
        skip,
        take: pageSizeNum,
      }),
      prisma.masterChangePasswordDB.count({ where }),
    ])
    ...
    console.error('GET master-change-password error:', error)
...
async function handlePost(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    if (!adviser || !usernameAG || !masterLogin || !oldPassword || !newPassword) {
      return res.status(400).json({ success: false, error: 'กรุณากรอกข้อมูลให้ครบถ้วน' })
    }

    const created = await prisma.$transaction(async (tx) => {
      const record = await tx.masterChangePasswordDB.create({
        data: {
    ...
    return res.status(201).json({
      ...
      message: 'สร้างคำขอเปลี่ยนรหัสผ่าน Master สำเร็จ',
    })
  } catch (error) {
    console.error('POST master-change-password error:', error)
...
async function handlePut(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    const existing = await prisma.masterChangePasswordDB.findFirst({
      where: { id },
    })
    if (!existing) {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const record = await tx.masterChangePasswordDB.update({
        where: { id },
        data: {
          ...(adviser && { adviser: String(adviser) }),
          ...(usernameAG && { usernameAG: String(usernameAG) }),
          ...(masterLogin && { masterLogin: String(masterLogin) }),
    ...
    return res.status(200).json({
      ...
      message: 'แก้ไขคำขอเปลี่ยนรหัสผ่าน Master สำเร็จ',
    })
  } catch (error) {
    console.error('PUT master-change-password error:', error)
...
async function handleDelete(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.masterChangePasswordDB.delete({
        where: { id },
      })
    })

    return res.status(200).json({ success: true, message: 'ลบคำขอเปลี่ยนรหัสผ่าน Master สำเร็จ' })
  } catch (error) {
    console.error('DELETE master-change-password error:', error)
