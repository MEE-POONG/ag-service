...
async function handleGet(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    if (id) {
      const record = await prisma.unblockAgentDB.findFirst({
        where: { id: String(id) },
      })
      if (!record) {
    ...
    const [records, total] = await Promise.all([
      prisma.unblockAgentDB.findMany({
        where,
        orderBy: { createdAt: 'desc' },
        skip,
        take: pageSizeNum,
      }),
      prisma.unblockAgentDB.count({ where }),
    ])
    ...
    console.error('GET unblock-agent error:', error)
...
async function handlePost(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    if (!adviser || !usernameAG || !newPassword || !position) {
      return res.status(400).json({ success: false, error: `กรุณากรอกข้อมูลให้ครบถ้วน (${adviser}, ${usernameAG}, ${newPassword}, ${position})` })
    }

    const created = await prisma.$transaction(async (tx) => {
      const record = await tx.unblockAgentDB.create({
        data: {
    ...
    return res.status(201).json({
      ...
      message: 'สร้างคำขอปลดบล็อก Agent สำเร็จ',
    })
  } catch (error) {
    console.error('POST unblock-agent error:', error)
...
async function handlePut(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    const existing = await prisma.unblockAgentDB.findFirst({
      where: { id },
    })
    if (!existing) {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const record = await tx.unblockAgentDB.update({
        where: { id },
        data: {
    ...
    return res.status(200).json({
      ...
      message: 'แก้ไขคำขอปลดบล็อก Agent สำเร็จ',
    })
  } catch (error) {
    console.error('PUT unblock-agent error:', error)
...
async function handleDelete(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.unblockAgentDB.delete({
        where: { id },
      })
    })

    return res.status(200).json({ success: true, message: 'ลบคำขอปลดบล็อก Agent สำเร็จ' })
  } catch (error) {
    console.error('DELETE unblock-agent error:', error)
