...
async function handleGet(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    if (id) {
      const record = await prisma.unblockMasterDB.findFirst({
        where: { id: String(id) },
      })
      if (!record) {
    ...
    const [records, total] = await Promise.all([
      prisma.unblockMasterDB.findMany({
        where,
        orderBy: { createdAt: 'desc' },
        skip,
        take: pageSizeNum,
      }),
      prisma.unblockMasterDB.count({ where }),
    ])
    ...
    console.error('GET unblock-master error:', error)
...
async function handlePost(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    const created = await prisma.$transaction(async (tx) => {
      const record = await tx.unblockMasterDB.create({
        data: {
    ...
    return res.status(201).json({
      ...
      message: 'สร้างคำขอปลดบล็อก Master สำเร็จ',
    })
  } catch (error) {
    console.error('POST unblock-master error:', error)
...
async function handlePut(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    const existing = await prisma.unblockMasterDB.findFirst({
      where: { id },
    })
    if (!existing) {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const record = await tx.unblockMasterDB.update({
        where: { id },
        data: {
    ...
    return res.status(200).json({
      ...
      message: 'แก้ไขคำขอปลดบล็อก Master สำเร็จ',
    })
  } catch (error) {
    console.error('PUT unblock-master error:', error)
...
async function handleDelete(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.unblockMasterDB.delete({
        where: { id },
      })
    })

    return res.status(200).json({ success: true, message: 'ลบคำขอปลดบล็อก Master สำเร็จ' })
  } catch (error) {
    console.error('DELETE unblock-master error:', error)
