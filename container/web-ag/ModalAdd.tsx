...
interface WebBaseModalAddProps {
  onSuccess: () => void;
}

const WebBaseModalAdd: React.FC<WebBaseModalAddProps> = ({
  onSuccess,
}) => {
  ...
  return (
    <>
      <Button onClick={() => setIsOpen(true)} className="inline-flex items-center px-2 py-1 text-base bg-blue-100 text-blue-700 border border-solid border-blue-700 hover:bg-blue-200 disabled:opacity-50 disabled:cursor-not-allowed rounded-full" size="sm">
        <FaPlus className="mr-2" /> เพิ่ม Web Base
      </Button>

      <Modal open={isOpen} onOpenChange={setIsOpen} size="md" closeOnOverlayClick closeOnEsc>
        <Modal.Header>
          <div>
            <Modal.Title>เพิ่ม Web Base</Modal.Title>
          </div>
