...
interface MenuWebModalAddProps {
  onSuccess: () => void;
}
...
const MenuWebModalAdd: React.FC<MenuWebModalAddProps> = ({
  ...
  const loadParentMenus = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        console.error('Failed to load parent menus:', response.data.error);
      }
    } catch (error) {
  ...
  const handleManagerChange = (value: string, isChecked: boolean) => {
    setFormData(prev => ({
      ...
      manager: isChecked
        ? [...(prev.manager || []), value]
        : (prev.manager || []).filter(item => item !== value)
    }));
  };

  const handleClose = () => {
    if (isSaving) return; // ถ้ากำลังบันทึกข้อมูลอยู่จะไม่ปิดหน้าต่าง
    setIsOpen(false);
    setFormData({
      name: '',
      description: '',
