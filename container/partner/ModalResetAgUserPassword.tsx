...
interface ModalResetAgUserPasswordProps {
  onSuccess?: () => void;
  data: AgUserDB;
}

const ModalResetAgUserPassword: React.FC<ModalResetAgUserPasswordProps> = ({ data, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    // Validation
    if (!form.adviser || !form.usernameAG || !form.newPassword || !form.position) {
    ...
    try {
      ...
      // const res = await axios.post('/api/reset-ag-user-password', form);
      // if (!res.data?.success) {
      //   throw new Error(res.data?.error || 'สร้างคำขอไม่สำเร็จ');
      // }

      toast.success("สร้างคำขอรีเซ็ตรหัสผ่าน AG User สำเร็จ (Demo)");
      onSuccess?.();
      setIsOpen(false);
      // Reset form
      setForm({
  ...
  return (
    <>
      <Button
        ...
        size="xs"
        className="!bg-orange-600 !text-white hover:!bg-orange-700 rounded-full px-3"
      >
        รีเซ็ตรหัสผ่าน
      </Button>
          ...
          <div>
            <Modal.Title>
              <span className="bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">
                สร้างคำขอรีเซ็ตรหัสผ่าน AG User
              </span>
            </Modal.Title>
          </div>
        ...
        <Modal.Footer>
          <div className="flex space-x-2">
            ...
            <Button
              ...
              disabled={loading}
              className="inline-flex items-center px-4 py-2 text-sm text-white bg-orange-600 rounded hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
...
export default ModalResetAgUserPassword;
