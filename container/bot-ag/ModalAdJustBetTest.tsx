...
interface ModalAdJustBetTestProps {
  data?: AdjustBet;
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
const ModalAdJustBetTest: React.FC<ModalAdJustBetTestProps> = ({
  data,
  isOpen,
  onClose,
  onSuccess,
  ...
  const saveCustomerAgent = async (customerId: string, agentId: string) => {
    try {
      await axios.post('/api/customer-agent', {
        customerId,
        agentId,
        note: `Auto-created from ModalAdJustBetTest for customer ${customerId}`
      });
    } catch (error) {
...
export default ModalAdJustBetTest;
